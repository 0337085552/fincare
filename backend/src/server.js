...
const { uploadReceiptToS3, getReceiptFromS3, deleteReceiptFromS3 } = require('./s3');
...
function withReceiptUrl(req, item) {
  if (!item) return item;

  return {
    ...item,
    receipt_url: item.receipt_key
      ? `${getBaseUrl(req)}/api/receipts/${encodeURIComponent(item.receipt_key)}`
      : null
  };
}
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    const [[wallets]] = await pool.query(`
      SELECT
        COALESCE(SUM(balance), 0) AS wallet_balance,
    ...
    ok(res, {
      ...
      wallets,
      budget,
      recent: recent.map(row => withReceiptUrl(req, row)),
      cashflow,
      categoryStats
    });
  } catch (error) {
...
app.get('/api/categories', async (req, res) => {
  try {
    ...
    fail(res, 500, 'Không tải được danh mục', error.message);
  }
});
...
app.post('/api/categories', async (req, res) => {
  try {
    ...
    fail(res, 500, 'Không tạo được danh mục', error.message);
  }
});
...
app.get('/api/wallets', async (req, res) => {
  try {
    ...
    fail(res, 500, 'Không tải được ví tiền', error.message);
  }
});
...
app.post('/api/wallets', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [name, type, toNumber(balance), note]
    );

    created(res, { id: result.insertId, name, type, balance: toNumber(balance), note });
  } catch (error) {
    fail(res, 500, 'Không tạo được ví', error.message);
  }
});
...
app.get('/api/budgets', async (req, res) => {
  try {
    ...
    fail(res, 500, 'Không tải được ngân sách', error.message);
  }
});
...
app.post('/api/budgets', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [name, category_id || null, toNumber(limit_amount), start_date || null, end_date || null, note]
    );

    created(res, { id: result.insertId });
  } catch (error) {
    fail(res, 500, 'Không tạo được ngân sách', error.message);
  }
});
...
app.get('/api/goals', async (req, res) => {
  try {
    ...
    fail(res, 500, 'Không tải được mục tiêu', error.message);
  }
});
...
app.post('/api/goals', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [name, toNumber(target_amount), toNumber(current_amount), deadline || null, note]
    );

    created(res, { id: result.insertId });
  } catch (error) {
    fail(res, 500, 'Không tạo được mục tiêu', error.message);
  }
});
...
app.get('/api/transactions', async (req, res) => {
  try {
    ...
    if (keyword) {
      where.push('(t.title LIKE ? OR t.note LIKE ? OR c.name LIKE ?)');
    ...
    ok(res, rows.map(row => withReceiptUrl(req, row)));
  } catch (error) {
    fail(res, 500, 'Không tải được giao dịch', error.message);
  }
});
...
app.post('/api/transactions', upload.single('receipt'), async (req, res) => {
  ...
  try {
    ...
    const receipt = req.file ? await uploadReceiptToS3(req.file) : null;

    const [result] = await connection.query(
      `INSERT INTO transactions
       (title, amount, type, transaction_date, category_id, wallet_id, budget_id, note, receipt_key, receipt_name, receipt_mime)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        title,
        value,
        ...
        budget_id || null,
        note,
        receipt ? receipt.key : null,
        receipt ? receipt.name : null,
        receipt ? receipt.mime : null
      ]
    );
    ...
    created(res, {
      id: result.insertId,
      receipt_url: receipt ? `${getBaseUrl(req)}/api/receipts/${encodeURIComponent(receipt.key)}` : null
    });
  } catch (error) {
...
app.delete('/api/transactions/:id', async (req, res) => {
  ...
  try {
    ...
    if (transaction.wallet_id) {
      ...
      await connection.query(
        'UPDATE wallets SET balance = balance + ? WHERE id = ?',
        [transaction.amount * sign, transaction.wallet_id]
      );
    }

    await connection.query('DELETE FROM transactions WHERE id = ?', [req.params.id]);

    if (transaction.receipt_key) {
      await deleteReceiptFromS3(transaction.receipt_key);
    }
...
app.get('/api/receipts/:key(*)', async (req, res) => {
  try {
    const key = req.params.key;
    const result = await getReceiptFromS3(key);
    ...
    fail(res, 404, 'Không tìm thấy ảnh hóa đơn', error.message);
  }
});
